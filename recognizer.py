...
REST_LABELS = {"nada", "reposo", "sin_sena", "sin_senas", "neutral", "idle"}
...
class SignRecognizer:
    ...
    def __init__(
        ...
        if MODEL_PATH.exists() and LABELS_PATH.exists():
            ...
            self.extractor = LandmarkExtractor(include_wrist_trajectory=uses_wrist_trajectory)
            ...
            self.extractor = LandmarkExtractor(include_wrist_trajectory=False)
    ...
    def _is_rest_label(label):
        normalized = label.casefold().strip().replace("-", "_").replace(" ", "_")
        return normalized in REST_LABELS
    ...
    def _predict_legacy(self, frame):
        features = self.get_legacy_landmarks(frame)
        X = self.scaler.transform([features])
        prediction = self.model.predict(X, verbose=0)[0]
        label = str(self.label_encoder.inverse_transform([prediction.argmax()])[0])
        confidence = float(np.max(prediction))
        if confidence < self.confidence_threshold or self._prediction_margin(prediction) < self.minimum_margin:
            return f"Detectando: {label}", confidence, False, False

        is_new_translation = label != self.committed_label
        self.committed_label = label
        return label, confidence, is_new_translation, True

    def predict(self, frame):
        if self.mode == "legacy":
            return self._predict_legacy(frame)
        ...
        X = resample_sequence(list(self.sequence), self.sequence_length)[None, ...]
        prediction = self.model.predict(X, verbose=0)[0]
        confidence = float(np.max(prediction))
        margin = self._prediction_margin(prediction)
        raw_label = self.labels[int(np.argmax(prediction))]
        ...
        if confidence < self.confidence_threshold or margin < self.minimum_margin:
            self.predictions.clear()
            return f"Detectando: {raw_label}", confidence, False, False

        self.predictions.append((raw_label, confidence))
        recent_labels = [label for label, _ in self.predictions]
        ...
        return raw_label, stable_confidence, is_new_translation, True
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                label, confidence, is_new_translation, is_stable = self.predict(frame)
...
def parse_args():
    ...
    parser.add_argument("--threshold", type=float, default=0.65, help="Confianza minima para aceptar una prediccion")
    parser.add_argument(
        "--margin",
        type=float,
        default=DEFAULT_MINIMUM_MARGIN,
        help="Diferencia minima entre la primera y segunda seña para aceptar una prediccion.",
    )
